...
create_site() {
    ...
    read -s -p "Enter WordPress admin password (Press Enter to generate random): " admin_pass_input
    echo
...
HOOK
  ...
  if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files | grep -qE '^certbot(\.service|-renew\.service|-renew\.timer)'; then
